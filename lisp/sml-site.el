...
(if (or
     (member sml-lisp-directory load-path)
     (member (concat sml-lisp-directory "/") load-path))
    ()                                  ;take no prisoners
...
;; use font lock instead
(setq sml-load-hook
      '(lambda() "Fontify SML." (require 'sml-font)))
(require 'sml-mode)

;; reduce tabbing standard from 4 to 2
